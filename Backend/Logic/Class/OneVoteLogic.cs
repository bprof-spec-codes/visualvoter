...
namespace Logic
{
    ...
    public class OneVoteLogic : IOneVoteLogic
    {
        ...
        public bool CreateOneVote(OneVote vote, string name)
        {
            try
            {
                if (string.IsNullOrEmpty(vote.voteGroup) || vote.voteGroup.ToLower() == "string") //If we didn't get a votegroup from the frontend for the submitted vote, we look it up on backend side.
                {
                    var associatedVoteGroup = allVotesLogic.GetOneVote(vote.VoteID).voteGroup;
                    vote.voteGroup = associatedVoteGroup;
                }
                if (string.IsNullOrEmpty(vote.submitterName) || vote.voteGroup.ToLower() == "string") // -||-
                {
